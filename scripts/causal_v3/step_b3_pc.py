"""Step B3: PC causal discovery on ultra-focused 8-variable set.

Output: reports/causal_v3/focused_b3/pc/
"""
...
REPORT_DIR = PROJECT_ROOT / "reports" / "causal_v3" / "focused_b3" / "pc"
...
def main():
    ...
    log.info("=" * 60)
    log.info("Step B3 PC  |  variables: %s", ALL_VARS)
    log.info("=" * 60)
    ...
    extract_parent_sets(primary["edge_df"], TIER3_NEXT).to_csv(REPORT_DIR / "parent_sets.csv", index=False)
    ...
    log.info("\nStep B3 PC complete. Results in: %s", REPORT_DIR)
