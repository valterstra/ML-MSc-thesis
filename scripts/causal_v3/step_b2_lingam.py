"""Step B2: DirectLiNGAM causal discovery on focused 11-variable set.

Output: reports/causal_v3/focused_b/lingam/
"""
...
from step_b2_vars import ALL_VARS, TIER2_ACTION, TIER3_NEXT, VAR_TIER


def build_lingam_prior_knowledge(var_names):
    """Prior knowledge matrix using local VAR_TIER.
    pk[i,j]=0 forbids vi->vj; pk[i,j]=-1 allows it.
    Only forward-in-time edges (lower tier -> higher tier) are allowed.
    """
    n = len(var_names)
...
REPORT_DIR  = PROJECT_ROOT / "reports" / "causal_v3" / "focused_b" / "lingam"
...
def extract_edges(B, var_names, threshold):
    """Extract edges from LiNGAM adjacency matrix B (B[i,j] = effect of j->i)."""
    rows = []
    for i, vi in enumerate(var_names):
        for j, vj in enumerate(var_names):
            if i == j:
...
def main():
    ...
    log.info("=" * 60)
    log.info("Step B2 LiNGAM  |  variables: %s", ALL_VARS)
    log.info("=" * 60)
    ...
    log.info("Prior knowledge: %d forbidden edges", n_forbidden)
    ...
    pd.DataFrame(B, index=ALL_VARS, columns=ALL_VARS).to_csv(
        REPORT_DIR / "weight_matrix.csv")

    log.info("\nStep B2 LiNGAM complete. Results in: %s", REPORT_DIR)
