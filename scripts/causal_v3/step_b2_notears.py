"""Step B2: NOTEARS causal discovery on focused 11-variable set.

Output: reports/causal_v3/focused_b/notears/
"""
...
def build_temporal_mask(var_names):
    """Post-hoc mask using local VAR_TIER (forbid j->i when tier(j) >= tier(i))."""
    n = len(var_names)
    mask = np.ones((n, n))
...
REPORT_DIR = PROJECT_ROOT / "reports" / "causal_v3" / "focused_b" / "notears"
...
def main():
    ...
    log.info("=" * 60)
    log.info("Step B2 NOTEARS  |  variables: %s", ALL_VARS)
    log.info("=" * 60)
    ...
    log.info("Temporal mask applied (%d forbidden entries)", int((temporal_mask == 0).sum()))
    ...
    log.info("\nStep B2 NOTEARS complete. Results in: %s", REPORT_DIR)
