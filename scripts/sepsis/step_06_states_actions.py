...
Faithful adaptation of ai_clinician/preprocessing/05_build_states_and_actions.py.
Changes: import paths only (ai_clinician.* -> careai.sepsis.*).

Checkpointing: every --checkpoint-every patients, results are flushed to disk.
Use --resume to skip stays already present in the output file after a crash.

Inputs:
  patient_states_imputed.csv, qstime.csv, demog.csv, fluid_mv.csv,
  vaso_mv.csv, preadm_fluid.csv, uo.csv
Outputs:
  states_actions.csv

Usage:
    source ../.venv/Scripts/activate
    python scripts/sepsis/step_06_states_actions.py \\
        ...
        > logs/step_06_states_actions.log 2>&1 &

    # Resume after crash:
    python scripts/sepsis/step_06_states_actions.py ... --resume
"""
...
def build_states_and_actions(df, qstime, inputMV, inputCV, inputpreadm,
    ...
    """
    Identical logic to AI-Clinician 05_build_states_and_actions.build_states_and_actions().
    Writes results incrementally every checkpoint_every stays.
    """
    ...
    for idx, icustayid in enumerate(tqdm(icustayidlist, desc='Building states and actions')):
        ...
        input2  = inputCV.loc[inputCV[C_ICUSTAYID] == icustayid, :] if inputCV is not None else None
        ...
        vaso2  = vasoCV.loc[vasoCV[C_ICUSTAYID] == icustayid, :] if vasoCV is not None else None
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=(
        'Bins patient states into time windows and adds I/O and vasopressor data.'))
