"""Step 13b -- MarkovSim control layer: planner baseline + DDQN policy."""
...
from careai.icu_readmit.markovsim.control.ddqn import DQNConfig, greedy_policy_from_model, train_ddqn
from careai.icu_readmit.markovsim.control.evaluation import (
    evaluate_policies,
    load_seed_episodes,
    make_planner_policy,
    make_random_policy,
...
from careai.icu_readmit.markovsim.control.planner import PlannerConfig
from careai.icu_readmit.markovsim.model import MarkovSimEnsemble
from careai.icu_readmit.markovsim.simulator import MarkovSimEnvironment
from careai.icu_readmit.rl.bandit import (
    BanditConfig,
...
from careai.icu_readmit.rl.networks import DuelingDQN


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Step 13b: MarkovSim control layer")
    ...
    def add_common(p):
        ...
        p.add_argument("--control-model-dir", default="models/icu_readmit/markovsim_control_selected_causal")
        p.add_argument("--report-dir", default="reports/icu_readmit/markovsim_control_selected_causal")
        ...
        p.add_argument("--terminal-stop-threshold", type=float, default=0.5)
        ...
        p.add_argument("--log", default="logs/step_13b_markovsim_control.log")
    ...
    ddqn_p = sub.add_parser("train-ddqn", help="Train DDQN against MarkovSim")
    add_common(ddqn_p)
    ...
    bandit_p = sub.add_parser("train-bandit", help="Train simple multi-armed bandit against MarkovSim")
    add_common(bandit_p)
    ...
    eval_p.add_argument("--ddqn-path", default="models/icu_readmit/markovsim_control_selected_causal/ddqn_model.pt")
    eval_p.add_argument("--bandit-path", default="models/icu_readmit/markovsim_control_selected_causal/bandit_model.json")
...
def planner_config_from_args(args) -> PlannerConfig:
    return PlannerConfig(
        ...
        terminal_stop_threshold=args.terminal_stop_threshold,
...
def run_evaluation(
    ...
    for split, split_seed in [("val", args.seed), ("test", args.seed + 1)]:
        ...
        traces.to_csv(report_dir / f"step_13b_markovsim_policy_traces_{split}.csv", index=False)
        terminal_rows = traces[traces["step"] == 0].copy()
        terminal_rows.to_csv(report_dir / f"step_13b_markovsim_episode_summary_{split}.csv", index=False)
        diagnostics = build_policy_diagnostics(traces)
        save_json(report_dir / f"step_13b_markovsim_diagnostics_{split}.json", diagnostics)
        logging.info("Split=%s summary: %s", split, split_summary)

    save_json(report_dir / "step_13b_markovsim_summary.json", summary_bundle)
...
def main() -> None:
    ...
    logging.info("Step 13b MarkovSim started. mode=%s device=%s smoke=%s", args.mode, device, args.smoke)
    ...
    logging.info("Step 13b MarkovSim complete in %.1f sec", time.time() - t0)
