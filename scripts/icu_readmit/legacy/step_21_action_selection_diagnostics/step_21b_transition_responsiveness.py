"""
Step 21b -- Legacy action-selection diagnostics: transition responsiveness.
...
This is the second half of two-stage variable selection:
  Step 21a: outcome relevance    -- which features predict readmission?
  Step 21b: transition relevance -- which features respond to actions?  (this script)
...
-------
  reports/icu_readmit/legacy/step_21_action_selection_diagnostics/step_21b_transition_responsiveness/
    ...
    step_21b_transition_responsiveness.json  -- features above action_share threshold + metadata
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out-dir', default=os.path.join(
        REPO_DIR, 'reports', 'icu_readmit', 'legacy', 'step_21_action_selection_diagnostics', 'step_21b_transition_responsiveness'))
    ...
    log_file = args.log or os.path.join(REPO_DIR, 'logs', 'step_21b_transition_responsiveness.log')
    ...
    logging.info("Step 21b started. input=%s", args.input)
    ...
    with open(os.path.join(args.out_dir, 'step_21b_transition_responsiveness.json'), 'w') as f:
    ...
    logging.info("Step 21b complete.")
