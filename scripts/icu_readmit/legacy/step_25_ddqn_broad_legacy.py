"""
Step 11 -- Dueling DDQN + SARSA physician baseline (broad state).

Chains from: step_10b output (rl_dataset_broad.parquet)
Produces:
...
Key differences from sepsis step_11:
  - Input: rl_dataset_broad.parquet (s_* / s_next_* columns precomputed, no reward shaping needed)
  - n_actions=32 (2^5 binary drug combos)
  - No autoencoder -- state is already z-scored and clipped to [-5, 5]
  - No reward clipping -- terminal +-15 signal must be preserved
  - reward_threshold=20 for Q-value regularisation (slightly above +-15)
  - state_cols extracted dynamically from parquet column names
"""
import argparse
import logging
import os
...
def main():
    parser = argparse.ArgumentParser(description="Step 11: DDQN + SARSA for ICU readmission")
    ...
    parser.add_argument("--model-dir", default=str(PROJECT_ROOT / "models" / "icu_readmit" / "legacy" / "step_25_33" / "continuous"),
                        help="Output directory for models")
    parser.add_argument("--dqn-steps", type=int, default=100000,
                        help="DDQN training steps")
    parser.add_argument("--sarsa-steps", type=int, default=80000,
                        help="SARSA physician training steps")
    parser.add_argument("--device", default="cpu",
                        help="PyTorch device (cpu or cuda)")
    parser.add_argument("--smoke", action="store_true",
                        help="Smoke test: 500 steps each, use smoke parquet if present")
    parser.add_argument("--log", default=str(PROJECT_ROOT / "logs" / "legacy" / "icu_readmit" / "step_25_ddqn_broad_legacy.log"),
                        help="Log file path")
    ...
    # Verify all next_state_cols exist
    missing = [c for c in next_state_cols if c not in df.columns]
    if missing:
        raise ValueError(f"Missing next-state columns: {missing[:5]} ...")

    n_state = len(state_cols)
    logging.info("State features: %d", n_state)
    logging.info("  First 5 state cols: %s", state_cols[:5])
    logging.info("  Actions: %d unique in dataset", df["a"].nunique())
    ...
    # ── Summary ───────────────────────────────────────────────────────
    dt = time.time() - t0
    logging.info("Step 11 complete. Total time: %.1f sec (%.1f min)", dt, dt / 60)
