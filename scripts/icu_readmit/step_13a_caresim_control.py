"""Step 13a -- CARE-Sim control layer: planner baseline + DDQN policy."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Step 13a: CARE-Sim control layer")
    sub = parser.add_subparsers(dest="mode", required=True)

    def add_common(p):
        p.add_argument("--data", default="data/processed/icu_readmit/rl_dataset_selected.parquet")
        p.add_argument("--model-dir", default="models/icu_readmit/caresim_selected_causal")
        p.add_argument("--control-model-dir", default="models/icu_readmit/caresim_control_selected_causal")
        p.add_argument("--report-dir", default="reports/icu_readmit/caresim_control_selected_causal")
        ...
        p.add_argument("--log", default="logs/step_13a_caresim_control.log")
    ...
    ddqn_p = sub.add_parser("train-ddqn", help="Train DDQN against CARE-Sim")
    add_common(ddqn_p)
...
def build_policy_diagnostics(traces):
    """Build per-policy and pairwise diagnostics from evaluation traces."""
...
def run_evaluation(
    ...
    for split, split_seed in [("val", args.seed), ("test", args.seed + 1)]:
        ...
        traces.to_csv(report_dir / f"step_13a_policy_traces_{split}.csv", index=False)
        terminal_rows = traces[traces["step"] == 0].copy()
        terminal_rows.to_csv(report_dir / f"step_13a_episode_summary_{split}.csv", index=False)
        diagnostics = build_policy_diagnostics(traces)
        save_json(report_dir / f"step_13a_diagnostics_{split}.json", diagnostics)
        logging.info("Split=%s summary: %s", split, split_summary)

    save_json(report_dir / "step_13a_summary.json", summary_bundle)
...
def main() -> None:
    ...
    logging.info("Step 13a started. mode=%s device=%s smoke=%s", args.mode, device, args.smoke)
    logging.info("Loading CARE-Sim ensemble from %s", args.model_dir)
    ensemble = CareSimEnsemble.from_dir(args.model_dir, device=device)
    severity_model = None
    terminal_outcome_model = None
    if args.use_severity_reward:
        state_dim = int(ensemble.models[0].state_dim)
        state_feature_names = ["Hb", "BUN", "Creatinine", "Phosphate", "HR", "Chloride", "age", "charlson_score", "prior_ed_visits_6m"][:state_dim]
    ...
    if args.use_terminal_readmit_reward:
        state_dim = int(ensemble.models[0].state_dim)
        state_feature_names = ["s_Hb", "s_BUN", "s_Creatinine", "s_Phosphate", "s_HR", "s_Chloride", "s_age", "s_charlson_score", "s_prior_ed_visits_6m"][:state_dim]
    ...
    logging.info("Step 13a complete in %.1f sec", time.time() - t0)
