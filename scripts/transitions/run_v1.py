...
from careai.qa.transition_report_v1 import generate_transition_qa, to_markdown
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build transition dataset v1.")
    parser.add_argument("--config", default=str(ROOT / "configs" / "transition_v1.yaml"))
    ...
    full_df = build_transitions(source, full_cfg)
    ...
    full_report = validate_transition_contract(full_df)
    sample_report = validate_transition_contract(sample_df)
