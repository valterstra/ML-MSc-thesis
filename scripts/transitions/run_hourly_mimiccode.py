...
from careai.mimiccode.extract_hourly import extract_hourly_from_postgres
from careai.qa.transition_report import generate_transition_qa, to_markdown
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the hourly transition dataset from mimic-code concepts.")
    parser.add_argument("--config", default=str(ROOT / "configs" / "transition_hourly_mimiccode.yaml"))
    ...
    sql_file = ROOT / "src" / "careai" / "mimiccode" / "sql" / "hourly_transition.sql"
    ...
    full_df = build_transitions(extracted, full_cfg)
    ...
    sample_df = build_transitions(sample_source, sample_cfg)
    ...
    full_report = validate_transition_contract(full_df, sofa_jump_threshold=int(cfg["outcome"]["sofa_jump_threshold"]))
    sample_report = validate_transition_contract(sample_df, sofa_jump_threshold=int(cfg["outcome"]["sofa_jump_threshold"]))
    ...
    qa = generate_transition_qa(sample_df)
