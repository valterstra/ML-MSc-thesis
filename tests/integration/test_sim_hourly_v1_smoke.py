...
def test_sim_hourly_script_smoke(tmp_path: Path) -> None:
    ...
    cfg = {
        ...
        "output": {"report_dir": str(out_dir), "prefix": "sim_hourly_v1"},
        "state": {"features_numeric": ["s_t_sofa", "s_t_mbp", "s_t_heart_rate", "s_t_resp_rate", "s_t_spo2", "s_t_creatinine", "s_t_bun", "s_t_age", "s_t_charlson"]},
    ...
    cmd = [sys.executable, str(root / "scripts" / "sim" / "run_hourly_v1.py"), "--config", str(cfg_path)]
    proc = subprocess.run(cmd, capture_output=True, text=True, check=False)
    assert proc.returncode == 0, proc.stderr
    assert (out_dir / "sim_hourly_v1_summary.json").exists()
    assert (out_dir / "sim_hourly_v1_policy_metrics.csv").exists()
    assert (out_dir / "sim_hourly_v1_trajectories.csv").exists()
