...
from careai.qa.transition_report import generate_transition_qa
...
def test_transition_build_from_fixture() -> None:
    cfg = {
    ...
    src = pd.DataFrame(
        {
            ...
            "s_t_spo2": [98, 97, 99],
            "s_t_gcs": [15, 14, 15],
            "s_t_urine_output_rate": [1.2, 1.1, 1.4],
    ...
    assert "s_t1_gcs" in out.columns
    assert "s_t1_urine_output_rate" in out.columns
    assert "s_t1_oxygen_delivery" in out.columns
