"""Build admission-step transition dataset v1."""
...
from careai.transitions.censoring import within_horizon_days
...
def _prepare(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return add_state_columns(out)


def build_transitions(df: pd.DataFrame, cfg: dict[str, Any]) -> pd.DataFrame:
    ...
    base["within_30d_next_admit"] = within_horizon_days(base["dischtime"], base["next_admittime"], days)
    ...
    has_next_within_horizon = base["within_30d_next_admit"] == 1
    base["s_t1_length"] = base["next_length"].where(has_next_within_horizon)
    base["s_t1_acuity"] = base["next_acuity"].where(has_next_within_horizon)
    base["s_t1_comorbidity"] = base["next_comorbidity"].where(has_next_within_horizon)
    base["s_t1_lace"] = base["next_lace"].where(has_next_within_horizon)
