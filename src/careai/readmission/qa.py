"""QA utilities for the readmission episode table."""
...
def generate_episode_qa(df: pd.DataFrame) -> dict[str, Any]:
    out = {
        "rows": int(len(df)),
...
def qa_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# Readmission Episode Table QA",
        "",
        f"- OK: {report.get('ok')}",
