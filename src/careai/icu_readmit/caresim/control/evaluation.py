"""Seed episode loading and simulator-based policy evaluation."""
...
from ..dataset import BLOC_COL, SPLIT_COL, STAY_COL, infer_schema_from_path
...
def load_seed_episodes(
    ...
    """Load reproducible seed histories from the parquet dataset."""
    state_cols, action_cols, _ = infer_schema_from_path(data_path)
...
def make_repeat_last_policy() -> PolicyFn:
    def _policy(env: CareSimEnvironment, obs_builder: ObservationBuilder, current_obs: np.ndarray, episode: SeedEpisode) -> dict:
...
def make_planner_policy(ensemble: CareSimEnsemble, planner_config: PlannerConfig, device: torch.device) -> PolicyFn:
    def _policy(env: CareSimEnvironment, obs_builder: ObservationBuilder, current_obs: np.ndarray, episode: SeedEpisode) -> dict:
...
def evaluate_policies(
    ...
    """Run policy rollouts in CARE-Sim and return summary + episode traces."""
    rows: list[dict] = []
